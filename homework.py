...
class CaloriesCalculator(Calculator):
    def get_calories_remained(self):
        ...
        if today_remains > 0:
            return (f'Сегодня можно съесть что-нибудь ещё, но с общей калорийностью не более {int(today_remains)} кКал')
        else:
            return 'Хватит есть!'
...
class CashCalculator(Calculator):
        ...
        def get_today_cash_remained(self, currency):
            ...
            if today_remains > 0:
                return (f'На сегодня осталось {today_remains_currency} {currency_name}')
            else:
                return (f'Денег нет, держись: твой долг - {today_remains_currency} {currency_name}')
